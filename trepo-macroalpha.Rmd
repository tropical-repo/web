...
bay (IB)inclued the two hypoxic sites.
...
```{r, eval=TRUE}
anova_week_even <- aov(pielou ~ factor(week) * site, alpha)
summary(anova_week_even)
```

There is a significant effect of site, week and their interaction on Pielou's Evenness.

```{r, echo=FALSE}
p <- ggplot(alpha, aes(x = week, y = pielou, group = site, color =site)) +
  geom_point(size = 1) +
  scale_colour_manual(values = swel_col) +
  ylab("Species Evenness'") + 
  xlab('week')+
...
```{r, eval=TRUE}
anova_week_H <- aov(shannon ~ week * site * season, alpha)
summary(anova_week_H)
```

Only site is significant. 

```{r, echo=FALSE}
p2 <- ggplot(alpha, aes(x = week, y = shannon, group = site, color =site)) +
  geom_point(size = 1) +
  scale_colour_manual(values = swel_col) +
...
```{r, eval=TRUE}
anova_week_rich <- aov(richness ~ week * site * season, alpha)
summary(anova_week_rich)
```

Only site is significant. No time interaction (via week)

```{r, echo=FALSE}
p3 <- ggplot(alpha, aes(x = week, y = richness, group = site, color =site)) +
  geom_point(size = 1) +
  scale_colour_manual(values = swel_col) +
  ylab("Species Richness") + 
  xlab('week')+
